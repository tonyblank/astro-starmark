...
export const StarMarkConfigSchema = z.object({
  ...
  ui: z.object({
     ...
     */
    categories: z.array(z.string()).default(['Bug', 'Feature Request', 'Question', 'Typo']),
 ...
 */
export function starmark(config: Partial<StarMarkConfig> = {}): AstroIntegration {
  return {
    name: 'starmark',
    hooks: {
      'astro:config:setup': ({ logger }) => {
        try {
          ...
          logger.info('StarMark integration loaded successfully (no-op for now)');
          
        } catch (error) {
          // Fail gracefully - don't break the build
          if (error instanceof z.ZodError) {
            logger.warn('StarMark integration configuration error:');
            logger.warn(JSON.stringify(error.errors));
            logger.warn('StarMark integration disabled due to configuration errors');
          } else {
            logger.warn('StarMark integration error:');
            logger.warn(String(error));
            logger.warn('StarMark integration disabled');
          }
        }
      },
      ...
      'astro:config:done': ({ config, logger }) => {
        if (config.output === 'static') {
          logger.warn('StarMark integration works best with server-side rendering (output: "server" or "hybrid")');
        }
      },
    },
  };
